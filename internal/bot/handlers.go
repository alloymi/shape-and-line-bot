...
func faqLevelHandler(bot *Bot, m *tgbotapi.Message) {
	bot.sendText(m.Chat.ID, "Мы будем рады помочь вам с выбором!\n\nЧтобы оценить ваш уровень и подобрать подходящий курс, расскажите о ваших актуальных целях. Также нужно посмотреть небольшое портфолио из 4–5 работ ссылкой на диск или файлами — это поможет нашим кураторам подсказать, какой курс будет для вас сейчас наиболее полезным.\n\nВсю подготовленную информацию и портфолио можете отправить нашему менеджеру @shapeandlinemanager — и мы поможем подобрать курс для вас!")
}
